 ...
 * Update URI: woocommerce-germanized-pro
 *
 * @author vendidero
 */
...
if ( ! class_exists( 'WooCommerce_Germanized_Pro' ) ) :
	...
	final class WooCommerce_Germanized_Pro {
		...
		public function __construct() {
			...
			add_action( 'storeabill_load_plugin_textdomain', array( $this, 'load_plugin_textdomain' ) );
		...
		public function load() {
			/**
			 * Additional dependency check after plugins_loaded which does check for
			 * actual instances and not active_plugins options (which may be outdated).
			 */
			if ( ! class_exists( 'WooCommerce_Germanized' ) || ! class_exists( 'WooCommerce' ) ) {
				if ( ! class_exists( 'WooCommerce' ) ) {
					add_action( 'admin_notices', array( WC_GZDP_Dependencies::instance(), 'dependencies_notice' ) );
				} elseif ( ! class_exists( 'WooCommerce_Germanized' ) ) {
					add_action( 'admin_notices', array( WC_GZDP_Dependencies::instance(), 'dependencies_gzd_notice' ) );
				}
			}
		...
		/**
		 * Prevent auto updating Germanized in case Germanized Pro has
		 * disabled auto updates and new Germanized version is not compatible with current Pro version.
		 *
		...
		public function prevent_dangerous_auto_updates( $should_update, $plugin ) {
			...
			$plugins_to_check = array(
				'woocommerce-germanized'     => array(
					'version'          => get_option( 'woocommerce_gzd_version', '1.0' ),
					'has_auto_updates' => false,
					'path'             => \Vendidero\Germanized\PluginsHelper::get_plugin_path_from_slug( 'woocommerce-germanized' ),
				),
				'woocommerce-germanized-pro' => array(
					'version'          => get_option( 'woocommerce_gzdp_version', '1.0' ),
					'has_auto_updates' => false,
					'path'             => \Vendidero\Germanized\PluginsHelper::get_plugin_path_from_slug( 'woocommerce-germanized-pro' ),
				),
			);

			$current_plugin_basename = basename( $plugin->plugin );
			$current_plugin_slug     = str_replace( '.php', '', $current_plugin_basename );
			$new_plugin_version      = wc_clean( $plugin->new_version );

			if ( ! in_array( $current_plugin_slug, array_keys( $plugins_to_check ), true ) ) {
				return $should_update;
			}

			$auto_updates_enabled = function_exists( 'wp_is_auto_update_enabled_for_type' ) ? wp_is_auto_update_enabled_for_type( 'plugin' ) : true;

			if ( $auto_updates_enabled ) {
				$auto_updates = (array) get_site_option( 'auto_update_plugins', array() );

				foreach ( $plugins_to_check as $plugin_slug => $plugin_data ) {
					if ( in_array( $plugin_data['path'], $auto_updates, true ) ) {
						$plugin_data['has_auto_updates'] = true;
					}
				}

				$plugins_to_check[ $current_plugin_slug ]['has_auto_updates'] = true;
			}

			if ( 'woocommerce-germanized' === $current_plugin_slug && ( ! $plugins_to_check['woocommerce-germanized-pro']['has_auto_updates'] || ! $this->has_valid_update_and_support_flat() ) ) {
				$max_currently_supported = WC_GZDP_Dependencies::instance()->get_wc_gzd_max_version_supported();

				if ( WC_GZDP_Dependencies::instance()->compare_versions( $max_currently_supported, $new_plugin_version, '<' ) ) {
					return false;
				}
			}
		...
		private function includes() {
			...
			if ( is_admin() ) {
				...
				if ( class_exists( 'Vendidero\Shiptastic\Admin\BulkActionHandler' ) ) {
		...
		public function load_modules() {
			...
			if ( ! did_action( 'setup_theme' ) ) {
				add_action( 'setup_theme', array( $this, 'load_theme_module' ), 1 );
			} else {
			...
			\Vendidero\Germanized\Pro\Shiptastic\Helper::init();
		}
		...
		public function has_valid_update_and_support_flat() {
			if ( function_exists( 'VD' ) ) {
				if ( $plugin = $this->get_vd_product() ) {
					return $plugin->is_registered() && ! $plugin->has_expired();
				}
			}
		...
		public function register_updates( $products ) {
			$product = vendidero_register_product( $this->plugin_file, '148' );

			if ( is_callable( array( 'Vendidero\Germanized\PluginsHelper', 'get_plugin_version' ) ) ) {
				$gzd_version = \Vendidero\Germanized\PluginsHelper::get_plugin_version( 'woocommerce-germanized' );
			} else {
				$gzd_version = get_option( 'woocommerce_gzd_version' );
			}

			if ( version_compare( $gzd_version, '3.20', '>=' ) ) {
				$product->single_license_page = admin_url( 'admin.php?page=wc-settings&tab=germanized-license' );
			}
		...
		public function load_plugin_textdomain() {
			...
			$custom_translation_path = trailingslashit( WP_LANG_DIR ) . 'woocommerce-germanized-pro/woocommerce-germanized-pro-' . $locale . '.mo';
			...
			if ( is_readable( $custom_translation_path ) ) {
				...
				load_textdomain( 'woocommerce-germanized-pro', $custom_translation_path );
			}
		}
