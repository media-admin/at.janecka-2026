...
use Vendidero\StoreaBill\Invoice\Shortcodes;
...
class AccountingHelper {
	...
	public static function init() {
		...
		add_filter( 'storeabill_default_vat_id', array( __CLASS__, 'add_default_vat_id' ) );
		add_filter( 'storeabill_default_contact_phone', array( __CLASS__, 'add_default_phone' ) );
		add_filter( 'storeabill_document_shortcodes', array( __CLASS__, 'register_document_shortcodes' ), 10 );
		add_filter( 'storeabill_invoice_shortcodes', array( __CLASS__, 'register_invoice_shortcodes' ), 10, 2 );
		...
		add_filter( 'storeabill_email_template_path', array( __CLASS__, 'register_gzd_email_template_path' ), 10, 1 );
		...
		/**
		 * Email small business notices
		 */
		add_action( 'storeabill_email_document_details', array( __CLASS__, 'email_small_business_notice' ), 50, 4 );
		...
		add_filter( 'storeabill_is_small_business', array( __CLASS__, 'is_small_business' ) );
	...
	public static function is_small_business( $is_small_business ) {
		if ( wc_gzd_is_small_business() ) {
			$is_small_business = true;
		}

		return $is_small_business;
	}

	public static function email_small_business_notice( $document, $sent_to_admin, $plain_text, $email ) {
		if ( is_a( $document, '\Vendidero\StoreaBill\Invoice\Invoice' ) && ! $sent_to_admin ) {
			if ( wc_gzd_is_small_business() ) {
	...
	public static function register_gzd_email_template_path( $path ) {
		/**
	...
	public static function enable_item_split_tax( $props, $item, $args ) {
		...
		if ( $order_item = $item->get_order_item() ) {
			if ( apply_filters( 'woocommerce_gzd_skip_order_item_split_tax_calculation', false, $order_item ) ) {
				$props['enable_split_tax'] = false;
			...
			if ( $order = $order_item->get_order() ) {
				...
				if ( 'fee' === $order_item->get_type() ) {
					...
					$fee_props->name      = $order_item->get_name();
					$fee_props->tax_class = $order_item->get_tax_class();
					$fee_props->taxable   = 'taxable' === $order_item->get_tax_status();
					$fee_props->amount    = $order_item->get_amount();
					$fee_props->id        = $order_item->get_meta( '_voucher_id' ) ? sanitize_title( $order_item->get_meta( '_voucher_id' ) ) : sanitize_title( $fee_props->name );
	...
	public static function maybe_treat_additional_costs_including_tax( $includes_tax, $order_item_type, $order ) {
		if ( in_array( $order_item_type, array( 'shipping', 'fee' ), true ) && ( self::enable_split_tax_calculation() || self::calculate_additional_costs_taxes_based_on_main_service() ) ) {
			...
			if ( ! empty( $meta ) ) {
				...
				$includes_tax                   = true;
				$order_version                  = $order->get_meta( '_gzd_version' ) ? $order->get_meta( '_gzd_version' ) : '1.0.0';
				$last_invoice_item_includes_tax = null;

				/**
				 * Do a safety check to prevent unnecessary cancellations by checking the tax status of additional costs within the
				 * latest, finalized invoice.
				 */
				if ( $last_invoice = $order->get_latest_finalized_invoice() ) {
					if ( $last_invoice->is_cancelable() ) {
						$last_items = $last_invoice->get_items( array( 'shipping', 'fee' ) );

						if ( ! empty( $last_items ) ) {
							$last_item = array_values( $last_items )[0];

							if ( $last_item ) {
								$last_invoice_item_includes_tax = $last_item->prices_include_tax();
							}
						}
					}
				}

				if ( is_bool( $last_invoice_item_includes_tax ) ) {
					$includes_tax = $last_invoice_item_includes_tax;
				} elseif ( version_compare( $order_version, '3.3.4', '>=' ) ) {
	...
	public static function order_date_of_service( $date_of_service, $order ) {
		if ( ! function_exists( 'wc_stc_get_shipment_order' ) ) {
			return $date_of_service;
		}

		if ( $shipment_order = wc_stc_get_shipment_order( $order->get_order() ) ) {
	...
	/**
	 * @param $shortcodes
	 * @param Shortcodes $shortcode_manager
	 *
	 * @return mixed
	 */
	public static function register_invoice_shortcodes( $shortcodes, $shortcode_manager ) {
		$shortcodes['has_photovoltaic_vat_exempt'] = function ( $atts, $content = '' ) use ( $shortcode_manager ) {
			$atts           = $shortcode_manager->parse_args( $atts );
			$has_vat_exempt = false;

			if ( $document = $shortcode_manager->get_document() ) {
				foreach ( $document->get_items( 'product' ) as $item ) {
					if ( $item->is_photovoltaic_system() && 'DE' === $document->get_taxable_country() && 0.0 === $item->get_total_tax() ) {
						$has_vat_exempt = true;
					}
				}
			}

			return $has_vat_exempt ? $content : '';
		};
	...
	public static function invoice_finalize_help_link() {
		return 'https://vendidero.de/doc/woocommerce-germanized/rechnungen-festschreiben';
	}

	public static function template_help_link() {
		return 'https://vendidero.de/doc/woocommerce-germanized/pdf-vorlagen-bearbeiten';
	}
	...
	public static function add_default_phone() {
		return get_option( 'widerruf_v2_telefon', '' );
	}

	public static function add_default_vat_id() {
		return get_option( 'woocommerce_gzdp_vat_requester_vat_id', '' );
	}
}
