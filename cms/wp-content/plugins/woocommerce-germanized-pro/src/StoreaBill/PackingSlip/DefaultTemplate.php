...
class DefaultTemplate extends Template {
	...
	public static function get_html() {
		...
		$preview_base_address = BusinessInformation::get_formatted_base_address( ' - ' );

		if ( Helper::is_active() ) {
			$preview_base_address = Countries::get_formatted_address( $preview->get_shipment()->get_sender_address(), ' - ', 'short' );
		}
		ob_start();
		?>
		...
		<div class="wp-block-storeabill-address sab-document-address has-text-align-left">
			<p class="address-heading">
				<span style="font-size:10px" class="has-inline-text-size">
					<span style="color:<?php echo esc_attr( self::get_light_color() ); ?>" class="has-inline-color"><span class="document-shortcode sab-tooltip" contenteditable="false" data-tooltip="<?php echo esc_attr__( 'Sender address', 'woocommerce-germanized-pro' ); ?>" data-shortcode="shipment?data=sender_address&format=address:line"><span class="editor-placeholder"></span><?php echo esc_html( $preview_base_address ); ?></span></span>
				</span>
			</p>
