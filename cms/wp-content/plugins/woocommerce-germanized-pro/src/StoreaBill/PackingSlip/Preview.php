...
class Preview extends PackingSlip implements Previewable {
	...
	public function __construct( $args = array() ) {
		...
		/**
		 * Shipment data store is only available when Shiptastic is enabled
		 */
		if ( Helper::is_active() ) {
			$shipment = new \Vendidero\Shiptastic\SimpleShipment();
			$shipment->set_id( 123 );
			$shipment->set_order_id( 4031 );
			...
			$this->shipment = new \Vendidero\Germanized\Pro\StoreaBill\PackingSlip\Shipment( $shipment );
			$this->shipment->sync( $this );
		} else {
			$this->set_address(
				array(
					'first_name' => _x( 'John', 'example-address', 'woocommerce-germanized-pro' ),
		...
		$item->set_quantity( 2 );
		$item->set_sku( '123' );
		$item->set_global_unique_id( '12345678' );
		$item->set_price( 10 );
		$item->set_total( 20 );
