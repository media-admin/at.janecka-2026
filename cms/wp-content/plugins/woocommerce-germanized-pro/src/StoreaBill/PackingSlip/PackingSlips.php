...
use Vendidero\Germanized\Pro\Shiptastic\Helper;
...
class PackingSlips {
	...
	public static function init() {
		...
		if ( AccountingHelper::is_enabled() ) {
			...
			add_action( 'woocommerce_shiptastic_shipment_deleted', array( __CLASS__, 'delete_packing_slip' ), 10, 2 );
			add_action( 'woocommerce_shiptastic_return_shipment_deleted', array( __CLASS__, 'delete_packing_slip' ), 10, 2 );
			add_action( 'woocommerce_gzdp_packing_slip_auto_sync_callback', array( __CLASS__, 'auto_sync_callback' ), 10 );
			add_filter( 'woocommerce_shiptastic_shipment_get_packing_slip_path', array( __CLASS__, 'shipment_packing_slip' ), 10, 2 );
			...
			add_filter( 'storeabill_packing_slip_plain_title', array( __CLASS__, 'adjust_plain_title' ), 10, 2 );
		}
		...
		if ( is_admin() ) {
			...
			if ( AccountingHelper::is_enabled() ) {
				add_action( 'woocommerce_shiptastic_meta_box_shipment_after_right_column', array( __CLASS__, 'packing_slip_meta_box' ), 20, 1 );

				add_filter( 'woocommerce_shiptastic_shipments_table_actions', array( __CLASS__, 'packing_slip_download_action' ), 10, 2 );
				add_filter( 'woocommerce_shiptastic_shipments_table_bulk_actions', array( __CLASS__, 'packing_slip_bulk_action' ), 10, 1 );
				add_filter( 'woocommerce_shiptastic_table_bulk_action_handlers', array( __CLASS__, 'register_packing_slip_bulk_handler' ) );
				add_filter( 'woocommerce_admin_order_actions', array( __CLASS__, 'order_download_actions' ), 10, 2 );
			}
		}
	}

	public static function on_shiptastic_init() {
	}

	/**
	 * @param string $title
	 * @param PackingSlip $document
	 *
	 * @return string
	 */
	...
	public static function attach_to_mail( $attachments, $email_id, $object, $email = false ) {
		...
		if ( apply_filters( 'woocommerce_gzdp_packing_slip_email_attach', ( in_array( $email_id, $email_ids, true ) ), $email_id, $object, $email ) ) {
			if ( is_a( $email, 'WC_STC_Email_Customer_Shipment' ) ) {
				if ( is_a( $email->shipment, '\Vendidero\Shiptastic\Shipment' ) ) {
	...
	public static function setup_automation() {
		...
		if ( 'yes' === get_option( 'woocommerce_gzdp_packing_slip_auto' ) && ! empty( $statuses ) ) {
			foreach ( $statuses as $status ) {
				add_action( 'woocommerce_shiptastic_shipment_status_' . $status, array( __CLASS__, 'queue_auto_sync_packing_slip' ), 10, 1 );
			}
	 ...
	 * @return string
	 */
	public static function shipment_packing_slip( $path, $shipment ) {
		if ( $packing_slip = self::get_packing_slip( $shipment ) ) {
			if ( $the_path = $packing_slip->get_path() ) {
				$path = $the_path;
			}
		}
	...
	/**
	 * @param \Vendidero\Shiptastic\Shipment $shipment
	 * @param boolean $only_for_changes
	 */
	public static function maybe_sync_packing_slip( $shipment, $only_for_changes = true ) {
		/**
		...
		if ( $syncable_shipment = self::get_shipment( $shipment ) ) {
			...
			if ( ! $packing_slip ) {
				...
				return self::sync_packing_slip( $shipment, true, false, $only_for_changes );
			}
		}
	...
	public static function get_packing_slip_label( $shipment_type = 'simple', $label_type = 'singular' ) {
		$label = sab_get_document_type_label( 'packing_slip', $label_type );

		if ( 'return' === $shipment_type ) {
			$return_labels = array(
				'singular' => __( 'Return slip', 'woocommerce-germanized-pro' ),
				'plural'   => __( 'Return slips', 'woocommerce-germanized-pro' ),
			);

			$label = $return_labels[ $label_type ];
		}
