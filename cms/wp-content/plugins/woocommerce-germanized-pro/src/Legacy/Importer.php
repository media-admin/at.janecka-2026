...
class Importer {
	...
	public static function delete( $id, $by_document = false ) {
		...
		} elseif ( $document = sab_get_document( $id ) ) {
	...
	protected function import_invoice_simple() {
		...
		} elseif ( $reference && ( $order_total == $this->document->get_total() && $order_tax_total == $this->document->get_total_tax() ) ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
		...
		} elseif ( 'wc-gzdp-paid' === $this->post->post_status ) {
				$this->document->set_payment_status( 'complete' );
		}
	...
	protected function import_invoice_simple_item_data() {
		...
		if ( ! get_post_meta( $this->post->ID, '_invoice_net_split_taxes', true ) ) {
		...
		if ( empty( $this->items_to_cancel ) ) {
			...
			foreach ( $this->items_to_cancel as $item_id => $item_data ) {
				if ( $parent_item = $this->get_parent()->get_item( $item_id ) ) {
					 ...
					 * being cancelled.
					 */
					if ( 0 != $parent_item->get_total() ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
	...
	protected function import_invoice_cancellation_refund() {
		...
		if ( $refund_order = $this->get_refund_order() ) {
			...
			if ( $this->document->get_total() == $total && $this->document->get_total_tax() == $total_tax ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
	...
	protected function import_packing_slip() {
		...
		if ( empty( $shipment_id ) || ! ( $shipment = wc_stc_get_shipment( $shipment_id ) ) ) {
	...
	protected function find_parent_item_by_total( $total ) {
		foreach ( $this->get_parent()->get_items() as $item ) {
			if ( is_callable( array( $item, 'get_total' ) ) ) {
				if ( $item->get_total() == $total ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
