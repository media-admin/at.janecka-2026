...
class WC_GZDP_Contract_Helper {
	...
	public function __construct() {
		...
		add_filter( 'pre_render_block', array( $this, 'hide_payment_info_from_block' ), 10, 2 );
		add_filter( 'pre_render_block', array( $this, 'add_payment_link_to_block' ), 10, 2 );
		...
		// Prevent Woo Subscriptions redirect to cart on initial payments.
		add_filter( 'wcs_setup_cart_for_subscription_initial_payment', array( $this, 'prevent_wcs_cart_redirect' ), 10 );
	}

	public function prevent_wcs_cart_redirect() {
		return false;
	}
	...
	public function maybe_register_placeholder_gateways( $load_gateways ) {
		if ( WC()->is_rest_api_request() ) {
			$is_wc_admin = isset( $_SERVER['REQUEST_URI'] ) ? false !== strpos( wp_unslash( $_SERVER['REQUEST_URI'] ), trailingslashit( rest_get_url_prefix() ) . 'wc-admin/' ) : false; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function gateway_filter( $available_gateways ) {
		if ( $this->is_checkout() ) {
			...
			foreach ( $available_gateways as $method => $available_gateway ) {
				if ( 'placeholder_' !== substr( $method, 0, 12 ) && $this->gateway_needs_placeholder( $method ) ) {
	...
	public function admin_hooks() {
		...
		add_action( 'manage_woocommerce_page_wc-orders_custom_column', array( $this, 'admin_hpos_order_table_title' ), 10, 2 );
	...
	public function confirm_order( $order_id ) {
		...
		$order->add_order_note( __( 'Order confirmed.', 'woocommerce-germanized-pro' ) );
	...
	public function add_payment_link_to_block( $result, $block ) {
		if ( 'woocommerce/order-confirmation-additional-information' === $block['blockName'] ) {
			remove_action( 'woocommerce_thankyou', 'woocommerce_gzd_template_order_pay_now_button', wc_gzd_get_hook_priority( 'order_pay_now_button' ) );
	...
	public function hide_payment_info_from_block( $result, $block ) {
		if ( in_array( $block['blockName'], array( 'woocommerce/order-confirmation-additional-information', 'woocommerce/order-confirmation-status' ), true ) ) {
			$order_id = absint( get_query_var( 'order-received' ) );
			$hide     = true;

			if ( $order_id ) {
				if ( $order = wc_get_order( $order_id ) ) {
					$hide = wc_gzdp_order_needs_confirmation( $order ) ? true : false;
				}
			}

			if ( $hide ) {
				$this->remove_payment_gateway_thankyou();
			}
		}
	...
	protected function remove_payment_gateway_thankyou() {
	...
	public function admin_hpos_order_table_title( $column_id, $order ) {
		if ( ! $order ) {
			return;
		}

		if ( ! wc_gzdp_order_needs_confirmation( $order ) || $order->has_status( 'cancelled' ) ) {
			return;
		}

		if ( 'order_number' === $column_id ) {
