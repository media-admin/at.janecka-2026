	...
	class WC_GZDP_Install {
		...
		public static function setup_redirect() {
			if ( get_option( '_wc_gzdp_setup_wizard_redirect' ) ) {
				...
				if ( isset( $_GET['page'] ) && 'wc-gzdp-setup' === wc_clean( wp_unslash( $_GET['page'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					return;
				}
		...
		public static function legacy_invoice_import( $limit, $after ) {
			...
			if ( empty( $invoices ) ) {
				...
				$logger->log( 'notice', 'Starting new batch legacy invoice import', array( 'source' => 'wc-gzdp-legacy-invoice-import' ) );
				...
				if ( $current_ids == $last_post_ids ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
					/**
		...
		public static function update() {
			...
			if ( version_compare( $current_db_version, '1.2.0', '<' ) ) {
				...
				self::upgrade_3_9_0();
			} elseif ( version_compare( $current_db_version, '4.2.0', '<' ) ) {
				self::upgrade_4_2_0();
			} elseif ( version_compare( $current_db_version, '4.2.4', '<' ) ) {
				self::upgrade_4_2_4();
			}

			if ( class_exists( '\Vendidero\Shiptastic\Caches\Helper' ) ) {
				$providers = \Vendidero\Shiptastic\ShippingProvider\Helper::instance();

				if ( is_callable( array( $providers, 'reset_providers' ) ) ) {
					\Vendidero\Shiptastic\ShippingProvider\Helper::instance()->reset_providers();
				} elseif ( $cache = \Vendidero\Shiptastic\Caches\Helper::get_cache_object( 'shipping-providers' ) ) {
					$cache->flush();
					$providers->shipping_providers = null;
				}
			} elseif ( class_exists( '\Vendidero\Germanized\Shipments\Caches\Helper' ) ) {
				$providers = \Vendidero\Germanized\Shipments\ShippingProvider\Helper::instance();
		...
		private static function upgrade_4_2_4() {
			if ( function_exists( 'wc_stc_get_shipping_provider' ) ) {
				if ( $provider = wc_stc_get_shipping_provider( 'dpd' ) ) {
		...
		private static function upgrade_4_2_0() {
			$status_settings = array(
				'woocommerce_gzdp_packing_slip_auto_statuses',
				'woocommerce_gzdp_commercial_invoice_auto_statuses',
			);

			foreach ( $status_settings as $option_name ) {
				$option_value = array_filter( (array) get_option( $option_name ) );

				if ( ! empty( $option_value ) ) {
					$new_option_value = array();

					foreach ( $option_value as $status ) {
						$new_option_value[] = str_replace( 'gzd-', '', $status );
					}
		...
		private static function upgrade_3_9_0() {
			...
			foreach ( $packing_settings as $option_name ) {
				$gzd_base_option_name = str_replace( 'woocommerce_gzdp_shipment_', 'woocommerce_shiptastic_', $option_name );

				if ( 'woocommerce_gzdp_enable_auto_shipment_packing' === $option_name ) {
					$gzd_base_option_name = 'woocommerce_shiptastic_enable_auto_packing';
				}
			...
			if ( is_callable( array( '\Vendidero\Shiptastic\Install', 'migrate_to_configuration_sets' ) ) ) {
				\Vendidero\Shiptastic\Install::migrate_to_configuration_sets( array( 'dpd', 'gls' ) );
			}
		}
		...
		public static function create_legacy_template( $document_type = 'invoice', $make_default = true ) {
			...
			if ( $margins_first_page != $margins ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
		...
		public static function upgrade_fonts_path_suffix() {
			...
			if ( file_exists( $path ) && file_exists( $new_gzdp_dir ) ) {
				...
				foreach ( $files as $file ) {
					...
					@rename( $file, $file_to_go ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.rename_rename
				}
			}
		}
		...
		public static function upgrade_invoice_path_suffix() {
			...
			if ( file_exists( $path ) && ! file_exists( $new_gzdp_dir ) ) {
				...
				if ( ! rename( $path, $new_gzdp_dir ) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.rename_rename
		...
		public static function create_options() {
			...
			foreach ( $options as $value ) {
				...
				if ( self::is_pro_option( $value['id'] ) && $value['default'] && ! $value['skip_install'] ) {
