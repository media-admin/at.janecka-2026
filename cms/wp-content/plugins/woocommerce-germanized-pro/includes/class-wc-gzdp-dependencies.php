...
class WC_GZDP_Dependencies {
	 ...
	 * @var string
	 */
	public $wc_gzd_minimum_version_required = '3.19';
	 ...
	 * @var string
	 */
	public $wc_gzd_maximum_version_supported = '4.0';
	...
	public function get_plugin_version( $plugin_slug, $fallback_version_option_name = '' ) {
		if ( is_callable( array( 'Vendidero\Germanized\PluginsHelper', 'get_plugin_version' ) ) ) {
			...
			$fallback_version_option_name = empty( $fallback_version_option_name ) ? sanitize_key( $plugin_slug . '_version' ) : $fallback_version_option_name;

			$version = $this->parse_version( get_option( $fallback_version_option_name, '1.0' ) );
	...
	public function get_woocommerce_gzd_version() {
		return $this->get_plugin_version( 'woocommerce-germanized', 'woocommerce_gzd_version' );
	}
