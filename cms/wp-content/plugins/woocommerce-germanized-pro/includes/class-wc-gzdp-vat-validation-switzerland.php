...
class WC_GZDP_VAT_Validation_Switzerland {
	...
	public function __construct( $options = array() ) {
		...
		if ( ! class_exists( 'SoapClient' ) ) {
			...
			WC_germanized_pro()->log( 'Missing SOAP Client.', 'error', 'vat-validation' );
		} else {
			try {
	...
	public function check( $country, $nr ) {
		...
		if ( $this->client ) {
			try {
				if ( apply_filters( 'woocommerce_gzdp_vat_validation_switzerland_uid_validation', false ) ) {
					$args = array(
						'uid' => $country . $nr,
					);

					$rs     = $this->client->ValidateUID( $args );
					$result = $rs->ValidateUIDResult; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				} else {
					$args = array(
					...
					$result = $rs->ValidateVatNumberResult; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
				}
		...
		return apply_filters( 'woocommerce_gzdp_vat_validation_switzerland_result', $this->valid, $country, $nr, $rs, $this->options, $this->errors );
	}
