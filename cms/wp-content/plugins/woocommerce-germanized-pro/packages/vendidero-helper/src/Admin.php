	...
	public static function product_status_notice() {
		...
		foreach ( Package::get_products( false ) as $product ) :
				...
				foreach ( $product->get_blog_ids() as $blog_id ) :
					...
					elseif ( $product->supports_renewals() && $product->has_expired( $blog_id ) && ! self::notice_is_hidden( 'expired', $notice_product_id ) ) :
						...
						<div class="vd-notice notice error <?php echo esc_attr( self::current_user_can_hide_notices() ? 'is-dismissible' : '' ); ?>" role="alert" data-id="expired" data-product_id="<?php echo esc_attr( $notice_product_id ); ?>">
							<p>
								<?php printf( esc_html_x( 'Your %1$s license for %2$s has expired on %3$s. %4$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', esc_html( $blog_info->blogname ), esc_html( $product->get_expiration_date( get_option( 'date_format' ), $blog_id ) ), '<a style="margin-left: 5px;" class="button button-primary wc-gzd-button" target="_blank" href="' . esc_url( $product->get_renewal_url( $blog_id ) ) . '">' . esc_html_x( 'renew now', 'vd-helper', 'woocommerce-germanized-pro' ) . '</a>' ); ?>
					...
					elseif ( $product->has_errors() ) :
						...
						<div class="vd-notice notice error" role="alert">
							<p>
								<?php echo wp_kses_post( sprintf( _x( 'We\'ve detected an issue with your %1$s <a href="%2$s">license</a>: %3$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', $product->get_license_page( $blog_id ), $error_message ) ); ?>
							</p>
						</div>
					<?php endif; ?>
				<?php endforeach; ?>
			...
			elseif ( $product->has_errors() ) :
				...
				<div class="vd-notice notice error" role="alert">
					<p>
						<?php echo wp_kses_post( sprintf( _x( 'We\'ve detected an issue with your %1$s <a href="%2$s">license</a>: %3$s', 'vd-helper', 'woocommerce-germanized-pro' ), '<strong>' . esc_attr( $product->Name ) . '</strong>', $product->get_license_page(), $error_message ) ); ?>
					</p>
				</div>
			<?php endif; ?>
		<?php endforeach; ?>
	...
	public static function process_register() {
		...
		if ( $product = Package::get_product( $file ) ) {
			if ( empty( $license_key ) ) {
				wp_send_json(
					array(
						'message' => _x( 'Please provide your license key.', 'vd-helper', 'woocommerce-germanized-pro' ),
					)
				);
		...
		wp_send_json(
			array(
				'message' => _x( 'There was an issue registering the license.', 'vd-helper', 'woocommerce-germanized-pro' ),
			)
		);
	}
