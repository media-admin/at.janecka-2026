...
function sab_maybe_prefix_invoice_type( $invoice_type = 'simple' ) {
	if ( empty( $invoice_type ) ) {
		return $invoice_type;
	}

	if ( 'simple' === $invoice_type ) {
		return 'invoice';
	} elseif ( 'invoice' !== $invoice_type && substr( $invoice_type, 0, 8 ) !== 'invoice_' ) {
		return 'invoice_' . $invoice_type;
	} else {
		return $invoice_type;
	}
}
 ...
 *
 * @return Invoice[]|Cancellation[]|Simple[] Invoice list
 */
function sab_get_invoices_by_order( $order_id, $types = array(), $reference_type = 'woocommerce' ) {
	$types          = array_filter( array_map( 'sab_maybe_prefix_invoice_type', ( empty( $types ) ? sab_get_document_types( '', 'accounting' ) : (array) $types ) ) );
	$reference_type = is_a( $order_id, '\Vendidero\StoreaBill\Interfaces\Order' ) ? $order_id->get_reference_type() : $reference_type;
	$order_id       = is_a( $order_id, '\Vendidero\StoreaBill\Interfaces\Order' ) ? $order_id->get_id() : $order_id;

	return sab_get_invoices(
		array(
 ...
 * @return Simple|null
 */
function sab_get_latest_finalized_invoice_by_order( $order_id ) {
	if ( $sab_order = \Vendidero\StoreaBill\WooCommerce\Helper::get_order( $order_id ) ) {
...
function sab_invoice_has_line_total_after_discounts( $invoice ) {
	if ( $template = $invoice->get_template() ) {
		if ( has_block( 'storeabill/item-line-total', $template->get_content() ) ) {
			$block_content = \Vendidero\StoreaBill\Editor\Helper::get_block_content( 'storeabill/item-line-total', $template->get_content(), true );

			if ( ! empty( $block_content ) ) {
				$parsed_block = parse_blocks( $block_content );

				if ( ! empty( $parsed_block ) ) {
					$item_line_total_block = $parsed_block[0];
					$attributes            = wp_parse_args(
						$item_line_total_block['attrs'],
						array(
							'discountTotalType' => 'before_discounts',
						)
					);

					if ( in_array( $attributes['discountTotalType'], array( 'after_discounts' ), true ) ) {
						return true;
					}
				}
			}
 ...
 * @param Invoice $invoice
 */
function sab_invoice_has_subtotal_after_discounts( $invoice ) {
	if ( $template = $invoice->get_template() ) {
