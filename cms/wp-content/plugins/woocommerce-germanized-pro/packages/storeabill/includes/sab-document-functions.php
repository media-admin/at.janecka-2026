 ...
 * @return array
 */
function sab_get_document_attachment_types( $document_type ) {
	$document_attachment_types = array();

	if ( $document_type = sab_get_document_type( $document_type ) ) {
		$document_attachment_types = $document_type->attachment_types;
	}

	/**
	 * Add or adjust available document attachment types.
	 *
	 * @param array $attachment_types The available document attachment types.
	 * @param string $document_type The document types
	 *
	 * @since 1.0.0
	 * @package Vendidero/StoreaBill
	 */
	return apply_filters( 'storeabill_document_attachment_types', $document_attachment_types, $document_type );
}
...
function sab_register_document_type( $type, $args = array() ) {
	...
	if ( 'accounting' === $args['group'] ) {
		$args['attachment_types'] = array_merge(
			array(
				'einvoice' => _x( 'eInvoice', 'storeabill-attachment-types', 'woocommerce-germanized-pro' ),
				'pdf'      => _x( 'Original PDF (not PDF/A)', 'storeabill-attachment-types', 'woocommerce-germanized-pro' ),
			),
 ...
 *
 * @return bool|\Vendidero\StoreaBill\Document\Attachment
 */
function sab_get_document_attachment( $attachment_id = 0, $attachment_type = 'other' ) {
	if ( ! did_action( 'storeabill_registered_core_document_types' ) ) {
		sab_doing_it_wrong( __FUNCTION__, 'sab_get_document_notice should not be called before document types are registered (storeabill_registered_core_document_types action)', '1.0' );
		return false;
	}

	return Factory::get_document_attachment( $attachment_id, $attachment_type );
}
...
function sab_upload_document( $filename, $stream, $relative = true, $force_override = false ) {
	try {
		...
		\Vendidero\StoreaBill\UploadManager::set_upload_mimes_filter();
		...
		\Vendidero\StoreaBill\UploadManager::unset_upload_mimes_filter();
		...
		if ( isset( $tmp['file'] ) ) {
			...
			if ( isset( $tmp['error'] ) && ! empty( $tmp['error'] ) ) {
				...
				\Vendidero\StoreaBill\Package::extended_log( $error_msg );

				throw new Exception( sprintf( _x( 'Error while uploading %1$s file: %1$s', 'storabill-core', 'storeabill' ), $filename, $error_msg ) );
			}

			throw new Exception( sprintf( _x( 'Error while uploading %1$s file', 'storabill-core', 'storeabill' ), $filename ) );
		}
	} catch ( Exception $e ) {
