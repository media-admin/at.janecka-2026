...
abstract class Document extends WC_Data_Store_WP implements WC_Object_Data_Store_Interface {
	...
	*/
	...
	public function create( &$document ) {
		...
		$document->set_revision( $document->get_revision( 'edit' ) + 1 );

		if ( '' === $document->get_secret() ) {
			$document->set_secret( $this->generate_secret( $document ) );
		}
		...
		$data = array(
			...
			'document_revision'         => $document->get_revision( 'edit' ),
			...
			'document_secret'           => $document->get_secret( 'edit' ),
	 ...
	 * @return string
	 */
	public function generate_secret( $document ) {
	...
	public function update( &$document ) {
		...
		if ( ! $document->get_secret() || $document->secret_has_expired() ) {
			$secret = $this->generate_secret( $document );
			$document->set_secret( $secret );
		}
		...
		if ( ! empty( $changed_props ) ) {
			$document->set_version( SAB_VERSION );
			$changed_props[] = 'version';

			if ( ! in_array( 'revision', $changed_props, true ) ) {
				$document->set_revision( $document->get_revision( 'edit' ) + 1 );
				$changed_props[] = 'revision';
			}
		}
	...
	public function delete( &$document, $force_delete = false ) {
		...
		if ( ! $force_delete ) {
			...
			$this->delete_attachments( $document );
		...
		if ( $data ) {
			$document->set_props(
				array(
					...
					'revision'          => $data->document_revision,
					'secret'            => $data->document_secret,
			...
			throw new Exception( esc_html_x( 'Invalid document.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}
	}
	...
	*/
	...
	/**
	 * Read attachments from the database for this document.
	 *
	 * @param \Vendidero\StoreaBill\Document\Document $document Document object.
	 *
	 * @return array
	 */
	public function read_attachments( $document, $type = '' ) {
		global $wpdb;

		// Get from cache if available.
		$attachments = 0 < $document->get_id() ? wp_cache_get( 'document-attachments-' . $document->get_id(), 'documents' ) : false;

		if ( false === $attachments ) {
			if ( $document->get_id() > 0 ) {
				$attachments = $wpdb->get_results(
					$wpdb->prepare( "SELECT * FROM {$wpdb->storeabill_document_attachments} WHERE document_id = %d ORDER BY document_attachment_id;", $document->get_id() )
				);
			} else {
				$attachments = array();
			}
			...
			foreach ( $attachments as $attachment ) {
				wp_cache_set( 'attachment-' . $attachment->document_attachment_id, $attachment, 'document-attachments' );
			}

			if ( 0 < $document->get_id() ) {
				wp_cache_set( 'document-attachments-' . $document->get_id(), $attachments, 'documents' );
			}
		}

		if ( ! empty( $type ) ) {
			$attachments = wp_list_filter( $attachments, array( 'document_attachment_type' => $type ) );
		}

		if ( ! empty( $attachments ) ) {
			$attachments = array_map(
				function ( $item ) {
					return sab_get_document_attachment( $item->document_attachment_id );
				},
				$attachments
			);
		} else {
	 ...
	 * @param \Vendidero\StoreaBill\Document\Document $document Document object.
	 */
	public function delete_attachments( $document ) {
		foreach ( $document->get_attachments() as $attachment ) {
	 ...
	 * @return int
	 */
	public function get_document_revision( $document ) {
		global $wpdb;

		$query = $wpdb->prepare( "SELECT document_revision FROM {$wpdb->storeabill_documents} WHERE document_id = %d", $document->get_id() );
