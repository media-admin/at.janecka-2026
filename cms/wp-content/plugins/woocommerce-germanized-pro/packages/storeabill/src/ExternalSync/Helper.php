...
class Helper {
			...
			foreach ( $handler::get_supported_object_types() as $object_type ) {
				if ( $handler->enable_auto_sync( $object_type ) ) {
					$sync_callback = function ( $object_id, $object = false ) use ( $handler_name, $object_type ) {
						 ...
						 */
						self::cancel_deferred_sync( $object, $handler_name );
	...
	public static function sync_callback( $object_id, $object_type, $handler, $reference_type = '' ) {
		...
		if ( is_a( $object, '\Vendidero\StoreaBill\Interfaces\ExternalSyncable' ) ) {
			$result = self::sync( $object, $handler, false );
	...
	public static function cancel_deferred_sync( $object, $handler, $retry = 0 ) {
		$retry = absint( $retry );
		$args  = array(
	 ...
	 * @return string|bool
	 */
	public static function sync_deferred( $object, $handler, $retry = 0 ) {
		$retry        = absint( $retry );
		$handler_name = is_a( $handler, '\Vendidero\StoreaBill\Interfaces\ExternalSync' ) ? $handler::get_name() : $handler;
		$args         = array(
			'object_id'      => $object->get_id(),
			'object_type'    => $object->get_type(),
		...
		// Add random offset to prevent race conditions
		$schedule_time = time() + wp_rand( 5, MINUTE_IN_SECONDS * 2 );

		if ( $retry > 0 ) {
			$args['retry']  = $retry;
			$schedule_time += min( (int) pow( 4, $retry ), 168 ) * HOUR_IN_SECONDS;
		}
	 ...
	 */
	public static function sync( $object, $handler, $is_manual_request = true ) {
		$errors = new \WP_Error();
		...
		try {
			...
			$result = $handler->sync( $object, $is_manual_request );
			...
			if ( is_wp_error( $result ) ) {
				...
				/**
				 * Retry sync on failure for deferred requests
				 */
				if ( ! $is_manual_request && apply_filters( 'storeabill_external_sync_allow_deferred_retry', true, $handler, $object ) ) {
					if ( $sync_data = $object->get_external_sync_handler_data( $handler::get_name() ) ) {
						$current_retries = $sync_data->get_retries();

						self::cancel_deferred_sync( $object, $handler, $current_retries );

						if ( $current_retries < apply_filters( 'storeabill_external_sync_max_sync_retries', 4, $handler, $object ) ) {
							$current_retry = $sync_data->get_retries() + 1;

							$object->update_external_sync_handler( $handler::get_name(), array( 'retries' => $current_retry ) );

							self::sync_deferred( $object, $handler, $current_retry );
						}
					}
				}
