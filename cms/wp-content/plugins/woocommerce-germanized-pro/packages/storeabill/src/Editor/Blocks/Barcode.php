...
class Barcode extends DynamicBlock {
	...
	public function render( $attributes = array(), $content = '' ) {
			...
			if ( is_a( $document, '\Vendidero\StoreaBill\Interfaces\Invoice' ) ) {
				...
				if ( ! empty( $bank_account['iban'] ) && ! empty( $bank_account['bic'] ) ) {
					if ( apply_filters( 'storeabill_invoice_display_girocode', ( ! $document->is_paid() && $document->get_total() > 0 ), $document ) ) {
						$subject  = apply_filters( 'storeabill_invoice_girocode_subject', $document->get_payment_reference(), $document );
		...
		if ( apply_filters( 'storeabill_barcode_disable_quiet_zone', false, $this, $document ) ) {
			$quiet_zone = 'quiet_zone_left="0" quiet_zone_right="0"';
		}

		if ( ! empty( $code ) ) {
			$this->content = '<barcode disableborder="1" error="M" style="' . sab_print_styles( $styles, false ) . '" class="barcode" code="' . esc_attr( $code ) . '" type="' . esc_attr( $barcode_type ) . '" size="' . esc_attr( $size ) . '" ' . $quiet_zone . '/>';
		}
