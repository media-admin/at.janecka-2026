...
class Helper {
	...
	public static function init() {
		...
		/**
		 * Starting with WP 7.0 global CSS variables are injected via theme.json default file. Override default font sizes.
		 */
		add_filter(
			...
			function ( $theme_data ) {
				...
				if ( $post && self::is_document_template( $post ) ) {
					$theme_data->update_with(
						array(
							'version'  => 3,
							'settings' => array(
								'typography' => array(
	...
	public static function override_theme_settings( $args, $block_editor_context ) {
		if ( is_a( $block_editor_context, 'WP_Block_Editor_Context' ) && $block_editor_context->post && self::is_document_template( $block_editor_context->post ) ) {
			unset( $args['colors'] );
				...
				if ( sab_document_type_supports( $template->get_document_type(), 'items' ) ) {
					$allowed_block_types = array_merge(
						array(
							...
							'storeabill/item-sku',
							'storeabill/item-global-unique-id',
