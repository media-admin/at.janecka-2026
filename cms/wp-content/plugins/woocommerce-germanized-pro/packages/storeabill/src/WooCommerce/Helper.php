...
class Helper {
	...
	public static function init() {
		...
		add_filter( 'woocommerce_orders_table_query_clauses', array( __CLASS__, 'search_hpos_order_invoices' ), 10, 3 );
		...
		Automation::init();
		\Vendidero\StoreaBill\eInvoice\Automation::init();
		Server::init();
	}
	...
	public static function register_webhook_topics( $topics ) {
		foreach ( sab_get_document_types() as $document_type ) {
			if ( $document_type_object = sab_get_document_type( $document_type ) ) {
				if ( $document_type_object->register_webhooks ) {
					$labels = $document_type_object->labels;

					$topics[ "{$document_type}.deleted" ]  = sprintf( _x( 'Deleted %s', 'storeabill-webhook-topic', 'woocommerce-germanized-pro' ), $labels['singular'] );
					$topics[ "{$document_type}.created" ]  = sprintf( _x( 'Created %s', 'storeabill-webhook-topic', 'woocommerce-germanized-pro' ), $labels['singular'] );
					$topics[ "{$document_type}.updated" ]  = sprintf( _x( 'Updated %s', 'storeabill-webhook-topic', 'woocommerce-germanized-pro' ), $labels['singular'] );
					$topics[ "{$document_type}.rendered" ] = sprintf( _x( 'Rendered %s', 'storeabill-webhook-topic', 'woocommerce-germanized-pro' ), $labels['singular'] );

					if ( 'accounting' === $document_type_object->group ) {
						$topics[ "{$document_type}.finalized" ] = sprintf( _x( 'Finalized %s', 'storeabill-webhook-topic', 'woocommerce-germanized-pro' ), $labels['singular'] );
					}

					if ( 'invoice' === $document_type ) {
						$topics[ "{$document_type}.cancelled" ] = sprintf( _x( 'Cancelled %s', 'storeabill-webhook-topic', 'woocommerce-germanized-pro' ), $labels['singular'] );
					}
				}
			}
		}
	...
	public static function register_webhooks( $topic_hooks, $webhook_handler ) {
		foreach ( sab_get_document_types() as $document_type ) {
			if ( $document_type_object = sab_get_document_type( $document_type ) ) {
				if ( $document_type_object->register_webhooks ) {
					$topic_hooks[ "{$document_type}.deleted" ]  = array(
						"storeabill_{$document_type}_deleted",
					);
					$topic_hooks[ "{$document_type}.updated" ]  = array(
						"storeabill_{$document_type}_updated",
					);
					$topic_hooks[ "{$document_type}.created" ]  = array(
						"storeabill_new_{$document_type}",
					);
					$topic_hooks[ "{$document_type}.rendered" ] = array(
						"storeabill_{$document_type}_rendered",
					);

					if ( 'accounting' === $document_type_object->group ) {
						$topic_hooks[ "{$document_type}.finalized" ] = array(
							"storeabill_{$document_type}_finalized",
						);
					}

					if ( 'invoice' === $document_type ) {
						$topic_hooks[ "{$document_type}.finalized" ] = array(
							"storeabill_{$document_type}_cancelled",
						);
					}
				}
			}
	...
	public static function attach_invoice_to_mail( $attachments, $email_id, $object, $email = false ) {
		...
		if ( apply_filters( 'storeabill_woo_attach_invoice_cancellation_refund_to_email', ( in_array( $email_id, array( 'customer_refunded_order', 'customer_partially_refunded_order' ), true ) ), $email_id, $object, $email ) ) {
	...
	public static function search_order_query( $args ) {
		if ( ! empty( $args['s'] ) ) {
			...
			foreach ( $invoices as $invoice ) {
				$order_ids[] = $invoice->get_reference_id();
			}
			...
			if ( ! empty( $order_ids ) ) {
				...
				$args['id'] = array_unique( array_merge( $order_ids, $args['id'] ) );
			}
		}
	...
	public static function search_hpos_order_invoices( $pieces, $datastore, $args ) {
		if ( isset( $args['s'] ) && ! empty( $args['s'] ) ) {
			...
			if ( ! empty( $order_ids ) ) {
				...
				$orders_table    = $datastore->get_table_name( 'orders' );
				$in_query        = "{$orders_table}.id IN ($ids)";
				$pieces['where'] = '1=1 AND ((' . $in_query . ') OR (' . $pieces['where'] . '))';
			}
		}
	...
	public static function customer_panel_download_button( $order_id ) {
		if ( $sab_order = self::get_order( $order_id ) ) {
			...
			foreach ( $sab_order->get_finalized_documents() as $document ) {
				if ( self::document_type_supports_customer_download( $document->get_type() ) ) {
					$documents[] = $document;
				}
			}
	...
	public static function validate_order( $order ) {
		...
		if ( $sab_order = self::get_order( $order_id ) ) {
			if ( ! $sab_order->needs_auto_validation() ) {
				$validate = false;
			}
		}
	...
	public static function sync_order_refund( $order_id, $refund_id ) {
		...
		sab_remove_all_hooks( 'woocommerce_new_order_note_data', 991 );
	}
	...
	public static function get_order_item( $order_item, $order ) {
		...
		return new $classname( $order_item, $order );
	}
	...
	public static function get_document_item_type( $order_item_type ) {
		...
		return apply_filters( 'storeabill_invoice_item_type_by_order_item_type', $type, $order_item_type );
	}
