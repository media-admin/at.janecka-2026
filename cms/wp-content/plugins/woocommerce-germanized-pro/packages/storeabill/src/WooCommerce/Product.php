...
class Product implements \Vendidero\StoreaBill\Interfaces\Product {
	...
	public function __construct( $product ) {
		...
		if ( ! is_a( $product, 'WC_Product' ) ) {
			throw new \Exception( esc_html_x( 'Invalid product.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}
	...
	public function get_global_unique_id() {
		return is_callable( array( $this->product, 'get_global_unique_id' ) ) ? $this->product->get_global_unique_id() : '';
	}
	...
	public function is_virtual() {
		return apply_filters( 'storeabill_product_is_virtual', $this->product->is_virtual(), $this->product, $this );
	}
