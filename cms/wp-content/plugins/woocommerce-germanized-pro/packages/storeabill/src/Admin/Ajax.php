...
class Ajax {
	...
	public static function add_ajax_events() {
		$ajax_events = array(
			...
			'external_sync',
			'send_document',
			'create_einvoice_load',
			'create_einvoice_submit',
	...
	public static function create_einvoice_load() {
		check_ajax_referer( 'sab-create-einvoice-load', 'security' );

		if ( ! self::get_request_data( 'reference_id' ) ) {
			wp_die( -1 );
		}

		$document_id  = absint( self::get_request_data( 'reference_id' ) );
		$display_type = sab_clean( self::get_request_data( 'display_type', 'table' ) );

		$response_error = array(
			'messages' => array( _x( 'Error while retrieving the document.', 'storeabill-core', 'woocommerce-germanized-pro' ) ),
		);
		...
		$fields = \Vendidero\StoreaBill\eInvoice\Helper::get_form_fields( $document );

		ob_start();
		include Package::get_path() . '/includes/admin/views/html-document-einvoice-backbone-form.php';
		$html = ob_get_clean();

		self::success(
			array(
				'fragments' => array(
					'.storeabill-create-einvoice' => '<div class="storeabill-create-einvoice">' . $html . '</div>',
				),
			)
		);
	}

	public static function create_einvoice_submit() {
		check_ajax_referer( 'sab-create-einvoice-submit', 'security' );
		...
		$document_id   = absint( self::get_request_data( 'document_id' ) );
		$display_type  = sab_clean( self::get_request_data( 'display_type', 'table' ) );
		$specification = sab_clean( self::get_request_data( 'einvoice_type' ) );

		if ( ! $document = sab_get_document( $document_id ) ) {
			self::error( $response_error );
		}

		if ( ! \Vendidero\StoreaBill\eInvoice\Helper::get_specification( $specification ) ) {
			self::error( $response_error );
		}

		$profile      = sab_clean( self::get_request_data( "einvoice_profile_{$specification}" ) );
		$merge        = sab_string_to_bool( sab_clean( self::get_request_data( 'einvoice_merge_pdf', 'no' ) ) );
		$profile_data = \Vendidero\StoreaBill\eInvoice\Helper::get_profile( $specification, $profile );
		...
		foreach ( $profile_data['fields'] as $field ) {
			...
			$field_id    = "{$profile}_" . $field['id'];
			$field_value = sab_clean( self::get_request_data( $field_id, $field['default'] ) );

			if ( $field['mandatory'] && empty( $field_value ) ) {
				$response_error['messages'][] = sprintf( _x( '%s is a mandatory field.', 'storeabill-core', 'woocommerce-germanized-pro' ), $field['label'] );
			}

			if ( ! empty( $field_value ) ) {
				$additional_meta[ $field['id'] ] = $field_value;
			}
		}
		...
		$result = $document->create_einvoice( $specification, $profile, $additional_meta );

		if ( is_wp_error( $result ) ) {
			foreach ( $result->get_error_messages() as $message ) {
				$response_error['messages'][] = $message;
			}
			...
			$selector     = 'table' === $display_type ? 'tr#document-' . $document_id . ' .column-actions' : 'div#sab-document-' . $document_id . ' .document-actions';
			$actions_html = self::get_document_actions_html( $document, $display_type );

			if ( 'table' === $display_type ) {
				$actions_html = '<td class="actions column-actions">' . $actions_html . '</td>';
			} else {
				$actions_html = '<div class="document-actions">' . $actions_html . '</div>';
			}

			self::success(
				array(
					'messages'  => array( _x( 'eInvoice created successfully.', 'storeabill-core', 'woocommerce-germanized-pro' ) ),
					'fragments' => array(
						$selector => $actions_html,
					),
				)
			);
		}
	 ...
	 * @param $step
	 */
	protected static function do_bulk_action( $handler, $type, $ids, $step, $ref_type = '', $referer = false ) {
		if ( 1 === $step ) {
			$handler->reset();
		}
		...
		if ( $handler->get_percent_complete() >= 100 ) {
			...
			self::success(
				array(
					'step'       => 'done',
					'percentage' => 100,
					'url'        => $handler->get_done_redirect_url( $referer ),
	...
	public static function handle_bulk_action() {
		...
		$referer  = self::get_request_data( 'referer', '' );
		...
		self::do_bulk_action( $handler, $type, $ids, $step, $ref_type, $referer );
	}
