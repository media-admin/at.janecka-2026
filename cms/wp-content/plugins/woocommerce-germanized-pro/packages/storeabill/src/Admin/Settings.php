...
class Settings {
	...
	public static function get_sections() {
		$sections = array(
			...
			'business'              => _x( 'Business Information', 'storeabill-core', 'woocommerce-germanized-pro' ),
		);

		if ( \Vendidero\StoreaBill\eInvoice\Helper::is_base_country_supported() ) {
			$sections['einvoice'] = _x( 'eInvoice', 'storeabill-core', 'woocommerce-germanized-pro' );
		}
	...
	public static function get_settings( $section = '' ) {
		...
		} elseif ( 'business' === $section ) {
			$settings = self::get_business_settings();
		} elseif ( 'einvoice' === $section && \Vendidero\StoreaBill\eInvoice\Helper::is_base_country_supported() ) {
			$settings = \Vendidero\StoreaBill\eInvoice\Helper::get_settings();
		}
	...
	protected static function get_business_settings() {
		$settings       = array();
		$inner_sections = array(
			'base_address' => _x( 'Address', 'storeabill-core', 'woocommerce-germanized-pro' ),
			'contact'      => _x( 'Contact', 'storeabill-core', 'woocommerce-germanized-pro' ),
			'legal'        => _x( 'Legal', 'storeabill-core', 'woocommerce-germanized-pro' ),
			'bank_account' => _x( 'Bank account', 'storeabill-core', 'woocommerce-germanized-pro' ),
		);

		foreach ( $inner_sections as $section_name => $label ) {
			$settings = array_merge(
				...
				array(
					array(
						'title' => $label,
						'type'  => 'title',
						'id'    => "business_information_{$section_name}_settings",
					),
				)
			);

			$fields_getter  = "get_{$section_name}_fields";
			$inner_settings = is_callable( array( '\Vendidero\StoreaBill\BusinessInformation', $fields_getter ) ) ? BusinessInformation::$fields_getter() : array();

			foreach ( $inner_settings as $setting_name => $setting ) {
