...
class Package {
	...
	public static function init() {
		...
		self::load_plugin_textdomain( true );
		self::define_tables();
		self::init_hooks();
		self::includes();
	 ...
	 */
	public static function load_plugin_textdomain( $on_load = false ) {
		if ( self::is_feature_plugin() ) {
			...
			unload_textdomain( 'storeabill', true );
	...
	/**
	 * Reload cached translations after locale change, e.g. when switching to site locale from a user's locale.
	 *
	 * @return void
	 */
	public static function on_change_locale() {
	 ...
	 * @param Document $document
	 */
	public static function is_document_setup( $document ) {
		if ( isset( $GLOBALS['document'] ) && $document === $GLOBALS['document'] ) {
			return true;
		}
	...
	public static function register_data_stores( $stores ) {
		$data_stores = apply_filters(
			...
			array(
				...
				'document_attachment'   => 'Vendidero\StoreaBill\DataStores\DocumentAttachment',
	...
	public static function is_debug_mode() {
		...
		return apply_filters( 'storeabill_is_debug_mode', $is_debug_mode );
	}

	public static function extended_log( $message, $source = '' ) {
		if ( apply_filters( 'storeabill_enabled_extended_log', self::is_debug_mode() ) ) {
			self::log( $message, 'info', ( ! empty( $source ) ? $source . '-extended-log' : 'extended-log' ) );
		}
	}
