...
class MpdfRenderer implements PDF {
	...
	public function __construct( $args ) {
		...
		try {
			...
			throw new DocumentRenderException( esc_html_x( 'Unable to setup mPDF.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}
	}
					...
					if ( isset( sab_document_styles()->registered[ $handle ] ) ) {
						...
						if ( isset( $item->src ) && ! empty( $item->src ) ) {
							...
							if ( 'http' !== substr( $path_or_url, 0, 4 ) ) {
								...
								} elseif ( $external_css_content = @file_get_contents( $path_or_url ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
									$external_css .= ' ' . $external_css_content;
								}
							}
						}
	...
	private function handle_error( $e, $callback, $args = array() ) {
		...
		throw new DocumentRenderException( esc_html( $message ) );
	}
