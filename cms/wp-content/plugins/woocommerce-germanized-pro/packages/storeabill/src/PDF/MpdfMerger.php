...
class MpdfMerger implements PDFMerge {
	...
	private function addPage( $pageNumber, $width = 210 ) {
		/**
		 * There seems to be an issue with groupXObject for PDF backgrounds (they are cropped at document margins when imported).
		 * To prevent this we need to explicitly set groupXObject to false.
		 */
		$pageId      = $this->_pdf->importPage( $pageNumber, PageBoundaries::CROP_BOX, false );
