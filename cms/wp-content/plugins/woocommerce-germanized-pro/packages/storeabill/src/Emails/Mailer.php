...
use Vendidero\StoreaBill\Invoice\SimplePreview;
...
class Mailer {
	...
	public static function init() {
		...
		add_action(
			...
			function ( $email_instance ) {
				if ( is_a( $email_instance, '\Vendidero\StoreaBill\Emails\Email' ) ) {
					if ( $preview = $email_instance->get_preview_object() ) {
						$email_instance->set_object( $preview );
					}
				}
	...
	public static function invoice_includes_subtotal_before_discounts( $invoice ) {
		return false === sab_invoice_has_subtotal_after_discounts( $invoice );
	}
