...
class Factory {
	...
	public static function get_document_item_type( $item_id ) {
		...
		$data = wp_cache_get( 'item-' . $item_id, 'document-items' );

		if ( false === $data ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->storeabill_document_items} WHERE document_item_id = %d LIMIT 1;", $item_id ) );
			wp_cache_set( 'item-' . $item_id, $data, 'document-items' );
		}
		...
		return $data->document_item_type;
	}
	 ...
	 */
	public static function get_document_attachment( $attachment_id = 0, $attachment_type = 'other' ) {
		...
		$classname = '\Vendidero\StoreaBill\Document\Attachment';

		if ( 'einvoice' === $attachment_type ) {
			$classname = '\Vendidero\StoreaBill\eInvoice\Attachment';
		}

		$classname = apply_filters( 'storeabill_document_attachment_classname', $classname, $attachment_id, $attachment_type );
	...
	public static function get_document_notice_type( $notice_id ) {
		...
		// Get from cache if available.
		$data = wp_cache_get( 'notice-' . $notice_id, 'document-notices' );

		if ( false === $data ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->storeabill_document_notices} WHERE document_notice_id = %d LIMIT 1;", $notice_id ) );
			wp_cache_set( 'notice-' . $notice_id, $data, 'document-notices' );
		}
		...
		return $data->document_notice_type;
	}
	...
	public static function get_document_attachment_type( $attachment_id ) {
		...
		// Get from cache if available.
		$data = wp_cache_get( 'attachment-' . $attachment_id, 'document-attachments' );

		if ( false === $data ) {
			$data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->storeabill_document_attachments} WHERE document_attachment_id = %d LIMIT 1;", $attachment_id ) );
			wp_cache_set( 'attachment-' . $attachment_id, $data, 'document-attachments' );
		}
		...
		return $data->document_attachment_type;
	}
	...
	public static function get_document_attachment_id( $attachment ) {
		...
		} elseif ( ! empty( $attachment->document_attachment_id ) ) {
