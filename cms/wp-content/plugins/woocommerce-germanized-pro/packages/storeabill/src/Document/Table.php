...
abstract class Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		$args = apply_filters( "{$this->get_hook_prefix()}query_args", $args, $this );
	...
	protected function get_views() {
		...
		foreach ( sab_get_document_statuses( $this->document_type ) as $status => $title ) {
			...
			$status_label = sprintf(
				translate_nooped_plural( _nx_noop( ( $title . ' <span class="count">(%s)</span>' ), ( $title . ' <span class="count">(%s)</span>' ), 'storeabill-core', 'woocommerce-germanized-pro' ), $num_documents[ $status ] ), // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralSingular, WordPress.WP.I18n.NonSingularStringLiteralPlural
