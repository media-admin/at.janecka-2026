...
class Shortcodes implements ShortcodeHandleable {
	...
	public function get_shortcodes() {
		$shortcodes = array(
			...
			'business_information'            => array( $this, 'business_information_data' ),
		...
		try {
			...
			if ( $ref->isPublic() ) {
				if ( $param_count > 0 && ! empty( $args ) ) {
					...
					$result = call_user_func( array( $object, $getter ) );
				}
			}
		} catch ( \Exception $e ) {
			$result = '';
	...
	public function business_information_data( $atts ) {
		$atts   = $this->parse_args( $atts );
		$getter = "get_{$atts['data']}";

		if ( is_callable( array( BusinessInformation::class, $getter ) ) ) {
			$value = $this->get_method_return( BusinessInformation::class, $getter, $atts['args'] );
		} else {
			$values = BusinessInformation::get_data();
			$value  = array_key_exists( $atts['data'], $values ) ? $values[ $atts['data'] ] : '';
		}

		return apply_filters( 'storeabill_business_information_shortcode_result', $this->format_result( $value, $atts, $this->get_document() ), $atts, $this->get_document(), $this );
	}
	...
	protected function compare( $types, $data, $comparison, $compare = 'OR' ) {
		...
		/**
		 * In case we are comparing with a bool, make sure to
		 * cast/transform possible comparison strings to bool first.
		 *
		 * WTF: In PHP, false != "false" but true == "true"
		 */
		if ( is_bool( $data ) ) {
			$comparison = sab_string_to_bool( $comparison );
		}
		...
		foreach ( $types as $c ) {
			...
			if ( 'e' === $c || 'equals' === $c || '==' === $c ) {
				$inner_show = $data == $comparison; // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
			} elseif ( 'ne' === $c || 'nequals' === $c || '!=' === $c ) {
				$inner_show = $data != $comparison; // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
			...
			} elseif ( 'like' === $c && is_string( $data ) ) {
				$inner_show = strstr( $data, (string) $comparison );
			} elseif ( 'in' === $c ) {
				$comparison = (array) $comparison;
				$data       = (array) $data;
	...
	protected function format_result( $data, $atts = array(), $object = false ) {
		...
		if ( ! empty( $sanitized_format ) && has_filter( "storeabill_format_{$sanitized_format}_shortcode" ) ) {
		...
		} elseif ( is_array( $return_data ) && ( 'address' === $format || 'address:' === substr( $format, 0, 8 ) ) ) {
			$address_format_args = explode( ':', $format );
			$address_break       = ! empty( $address_format_args[1] ) ? $address_format_args[1] : 'break';
			$include_company     = ! empty( $address_format_args[2] ) ? sab_string_to_bool( $address_format_args[2] ) : true;
			$address_type        = 'complete';

			if ( ! $include_company ) {
				$return_data['company'] = '';
			}

			if ( ! empty( $return_data['company'] ) ) {
				$return_data['first_name'] = '';
				$return_data['last_name']  = '';
			}

			if ( 'line' === $address_break ) {
				$address_type = 'short';
			}

			$return_data = Countries::get_formatted_address( $return_data, 'line' === $address_break ? ' - ' : '<br>', $address_type );
			...
			/**
			 * Do not convert line breaks to br tags in case the shortcode result
			 * is already a html string.
			 */
			if ( strip_tags( $data ) !== $data ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.strip_tags_strip_tags
				$return_data = wp_kses_post( wptexturize( $data ) );
			} else {
		...
		if ( ! $has_applied_format && ! empty( $format ) ) {
			if ( 'price' === $format ) {
				...
				if ( is_numeric( $return_data ) && (int) $return_data == $return_data ) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
					...
					if ( get_option( 'timezone_string' ) ) {
						...
						$datetime->set_utc_offset( sab_timezone_offset() );
					}
				}
