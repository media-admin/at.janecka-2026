...
class AsyncExporter {
	...
	public function __construct( $id, $args = array() ) {
		...
		if ( $transient = get_option( "sab_export_{$this->get_id()}" ) ) {
		...
		if ( ! $this->exporter = sab_get_document_type_exporter( $this->args['document_type'], $this->args['type'] ) ) {
			throw new \Exception( esc_html_x( 'No applicable exporter found.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}

		$filename = $id . '-export.' . $this->exporter->get_file_extension();

		if ( empty( $this->args['start_date'] ) ) {
			throw new \Exception( esc_html_x( 'Please make sure to provide a valid start date.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}

		if ( empty( $this->args['end_date'] ) ) {
			throw new \Exception( esc_html_x( 'Please make sure to provide a valid end date.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}
		...
		if ( $start_date && $start_date > $today ) {
			throw new \Exception( esc_html_x( 'Please choose a start date from the past.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}

		if ( $start_date && $end_date && $start_date > $end_date ) {
			throw new \Exception( esc_html_x( 'The end date must be after the start date.', 'storeabill-core', 'woocommerce-germanized-pro' ) );
		}
	...
	public function exists() {
		return get_option( "sab_export_{$this->get_id()}", false );
	}
	...
	public function delete() {
		...
		delete_option( "sab_export_{$this->get_id()}" );
	}
