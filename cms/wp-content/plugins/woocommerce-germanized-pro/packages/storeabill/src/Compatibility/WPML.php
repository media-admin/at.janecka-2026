...
class WPML implements Compatibility {
	...
	public static function init() {
		...
		/**
		 * Force native WP Editor for translating document templates.
		 */
		add_filter(
			'get_post_metadata',
			function ( $metadata, $object_id, $meta_key, $single ) {
				if ( '_wpml_post_translation_editor_native' === $meta_key && 'document_template' === get_post_type( $object_id ) && apply_filters( 'storeabill_wpml_force_native_editor_for_document_templates', true ) ) {
					return 'yes';
				}
		...
		add_filter( 'storeabill_woo_order_currency_exchange_rate', array( __CLASS__, 'get_currency_exchange_rate' ), 10, 4 );
		add_filter( 'storeabill_woo_order_price_formatting_args', array( __CLASS__, 'get_price_formatting_args' ), 10, 4 );
	 ...
	 * @return float
	 */
	public static function get_currency_exchange_rate( $exchange_rate, $order_currency, $base_currency, $order ) {
		global $woocommerce_wpml;

		if ( $order_currency !== $base_currency ) {
			if ( is_callable( array( $woocommerce_wpml, 'get_multi_currency' ) ) ) {
				$rates = $woocommerce_wpml->get_multi_currency()->get_exchange_rates();

				if ( isset( $rates[ $order_currency ] ) ) {
					$exchange_rate = (float) $rates[ $order_currency ];
				}
			}
		}
	 ...
	 * @return array
	 */
	public static function get_price_formatting_args( $price_formatting_args, $order_currency, $base_currency, $order ) {
		global $woocommerce_wpml;

		if ( $order_currency !== $base_currency ) {
			if ( is_callable( array( $woocommerce_wpml, 'get_multi_currency' ) ) ) {
				$currency_details = $woocommerce_wpml->get_multi_currency()->get_currency_details_by_code( $order_currency );

				if ( $currency_details ) {
					$price_formatting_args['decimal_separator']  = $currency_details['decimal_sep'];
					$price_formatting_args['thousand_separator'] = $currency_details['thousand_sep'];
					$price_formatting_args['decimals']           = $currency_details['num_decimals'];
					$price_formatting_args['price_format']       = sab_get_price_format_by_position( $currency_details['position'] );
				}
			}
		}
	...
	public static function switch_to_locale( $locale ) {
		...
		if ( version_compare( $wp_version, '6.7', '>=' ) ) {
			self::init_dynamic_translations();
		} else {
	...
	protected static function init_dynamic_translations() {
		/**
