...
class Install {
	...
	private static function create_tables() {
		...
		Package::extended_log( 'Db delta result: ' . sab_print_r( $db_delta_result, true ) );
	...
	private static function create_upload_dir() {
		...
		if ( ! @file_exists( trailingslashit( $dir['basedir'] ) . '.htaccess' ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			...
			@file_put_contents( trailingslashit( $dir['basedir'] ) . '.htaccess', $content ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.file_system_operations_file_put_contents
		}
		...
		if ( ! @is_dir( $font_dir ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
			@mkdir( $font_dir ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged, WordPress.WP.AlternativeFunctions.file_system_operations_mkdir
		}
...
CREATE TABLE {$wpdb->prefix}storeabill_document_attachments (
  document_attachment_id bigint(20) unsigned NOT NULL auto_increment,
  document_attachment_type varchar(200) NOT NULL DEFAULT '',
  document_attachment_relative_path varchar(260) NOT NULL DEFAULT '',
  document_attachment_name text NOT NULL,
  document_attachment_extension varchar(260) NOT NULL DEFAULT '',
  document_attachment_date_created datetime default NULL,
  document_attachment_date_created_gmt datetime default NULL,
  document_id bigint(20) unsigned NOT NULL,
  PRIMARY KEY  (document_attachment_id),
  KEY document_id (document_id)
) $collate;
CREATE TABLE {$wpdb->prefix}storeabill_document_attachmentmeta (
  meta_id bigint(20) unsigned NOT NULL auto_increment,
  storeabill_document_attachment_id bigint(20) unsigned NOT NULL,
  meta_key varchar(255) default NULL,
  meta_value longtext NULL,
  PRIMARY KEY  (meta_id),
  KEY storeabill_document_attachment_id (storeabill_document_attachment_id),
  KEY meta_key (meta_key(32))
) $collate;
CREATE TABLE {$wpdb->prefix}storeabill_documents (
  ...
  document_revision bigint(20) unsigned NOT NULL DEFAULT 0,
  document_type varchar(200) NOT NULL DEFAULT '',
  document_secret varchar(200) NOT NULL DEFAULT '',
