...
use Vendidero\StoreaBill\eInvoice\Helper;
...
abstract class Invoice extends Document implements \Vendidero\StoreaBill\Interfaces\Invoice, TotalsContainable, Discountable {
	...
	public function finalize( $defer_render = false ) {
		...
		if ( $updated && $this->is_finalized() ) {
			 ...
			 * construct an individual prefix based on the current invoice type.
			 *
			 * Example hook name: storeabill_invoice_finalized
			 *
			 * @param Invoice $invoice The invoice object.
			 *
			 * @since 1.0.0
			 * @package Vendidero/StoreaBill
			 */
			do_action( "{$this->get_general_hook_prefix()}finalized", $this );
	...
	protected function get_render_stream( $is_preview = false ) {
		$stream = parent::get_render_stream( $is_preview );

		if ( ! is_wp_error( $stream ) && $is_preview ) {
			$error = new \WP_Error();

			foreach ( $this->get_einvoices() as $e_invoice ) {
				$result = $e_invoice->generate( $stream, $is_preview );

				if ( is_wp_error( $result ) ) {
					foreach ( $result->get_error_messages() as $msg ) {
						$error->add( 'render-einvoice-error', $msg );
					}
				} elseif ( $e_invoice->has_pdf_merge() && ! empty( $stream ) ) {
					$merged_pdf_stream = $e_invoice->get_merged_pdf( $stream, $result );

					if ( $merged_pdf_stream ) {
						$stream = $merged_pdf_stream;
						break;
					}
				}
			}
	...
	public function get_primary_path( $context = 'default' ) {
		$primary_path = parent::get_primary_path( $context );

		if ( $this->is_finalized() && ! in_array( $context, array( 'bulk_merge', 'print' ), true ) ) {
			$adjust_primary_invoice = ! $this->has_pdf_attachment();

			foreach ( $this->get_einvoices() as $e_invoice ) {
				if ( apply_filters( "{$this->get_general_hook_prefix()}einvoice_is_primary_file", $adjust_primary_invoice && ! $e_invoice->has_pdf_merge(), $e_invoice, $this, $context ) ) {
					$primary_path = $e_invoice->get_path();
				}
			}
		}
	...
	protected function update_file( $stream ) {
		$path = parent::update_file( $stream );
		...
		foreach ( $this->get_einvoices() as $e_invoice ) {
			$result = $e_invoice->generate( $stream );

			if ( is_wp_error( $result ) ) {
				foreach ( $result->get_error_messages() as $msg ) {
					$error->add( 'render-einvoice-error', $msg );
				}
			} elseif ( $e_invoice->has_pdf_merge() && ! $has_merged && ! empty( $stream ) ) {
				$merged_pdf_stream = $e_invoice->get_merged_pdf( $stream, $result );

				if ( $merged_pdf_stream ) {
					$merge_result = $this->update_pimary_file( $merged_pdf_stream );

					if ( is_wp_error( $merge_result ) ) {
						$error->add( 'render-einvoice-error', $merge_result->get_error_message() );
					} else {
						$has_merged = true;
					}
				}
	...
	public function get_totals( $type = '' ) {
				...
				foreach ( $taxes as $tax_total ) {
					...
					$document_totals[] = new Total(
						...
						array(
							...
							'placeholders' => array(
								'{rate}'           => $tax_total->get_tax_rate()->get_formatted_percent(),
		...
		return apply_filters( "{$this->get_hook_prefix()}_totals", $document_totals, $this, $total_types );
	}
	...
	public function get_currency( $context = 'view' ) {
		...
		if ( 'view' === $context && empty( $currency ) ) {
	...
	public function get_base_currency( $context = 'view' ) {
		$base_currency = $this->get_prop( 'base_currency', $context );

		if ( 'view' === $context && empty( $base_currency ) ) {
	...
	/**
	 * Get currency exchange rate (in case differs from base currency).
	 *
	 * @param string $context
	 *
	 * @return string The currency code.
	...
	/**
	 * Get price formatting args
	 *
	 * @param string $context
	 *
	 * @return array Price format.
	 */
	public function get_price_formatting_args( $context = 'view' ) {
		$price_formatting_args = $this->get_prop( 'price_formatting_args', $context );

		if ( 'view' === $context ) {
			$price_formatting_args = wp_parse_args(
				(array) $price_formatting_args,
				array(
	...
	/**
	 * @return \Vendidero\StoreaBill\eInvoice\Attachment[]
	 */
	public function get_einvoices() {
		return $this->get_attachments( 'einvoice' );
	}
	 ...
	 * @param $profile
	 *
	 * @return false|\Vendidero\StoreaBill\eInvoice\Attachment
	 */
	public function get_einvoice( $specification, $profile ) {
		$invoice = false;

		foreach ( $this->get_einvoices() as $e_invoice ) {
			if ( $specification === $e_invoice->get_specification() && $profile === $e_invoice->get_profile() ) {
				$invoice = $e_invoice;
				break;
			}
		}
	...
	public function has_einvoice( $specification, $profile ) {
		return $this->get_einvoice( $specification, $profile ) ? true : false;
	}

	public function has_einvoices() {
		return $this->get_einvoices() ? true : false;
	}

	/**
	 * @param $specification
	 * @param $profile
	 * @param $additional_meta
	 *
	 * @return \Vendidero\StoreaBill\eInvoice\Attachment|WP_Error
	 */
	public function create_einvoice( $specification, $profile, $additional_meta = array() ) {
		$error           = new WP_Error();
		$additional_meta = wp_parse_args(
			$additional_meta,
			array(
				'merge_pdf' => false,
			)
		...
		if ( $this->has_einvoice( $specification, $profile ) ) {
			$error->add( 'einvoice-create', _x( 'An einvoice of this profile already exists.', 'storeabill-core', 'woocommerce-germanized-pro' ) );

			return $error;
		}

		if ( ! $profile_data = Helper::get_profile( $specification, $profile ) ) {
			$error->add( 'einvoice-create', _x( 'This profile does not exist.', 'storeabill-core', 'woocommerce-germanized-pro' ) );

			return $error;
		}

		$e_invoice = sab_get_document_attachment( 0, 'einvoice' );
		$e_invoice->set_document( $this );
		$e_invoice->set_specification( $specification );
		$e_invoice->set_profile( $profile );
		$e_invoice->set_merge_pdf( $additional_meta['merge_pdf'] );
		$e_invoice->set_extension( $profile_data['extension'] );
		...
		foreach ( $profile_data['fields'] as $field ) {
			...
			$field['default']          = apply_filters( "{$this->get_general_hook_prefix()}einvoice_meta_{$field['id']}_default", $field['default'], $e_invoice, $this );
			$meta_data[ $field['id'] ] = isset( $additional_meta[ $field['id'] ] ) ? $additional_meta[ $field['id'] ] : $field['default'];
		}

		$meta_data = apply_filters( "{$this->get_general_hook_prefix()}einvoice_additional_meta", $meta_data, $e_invoice, $this );

		foreach ( $meta_data as $meta_key => $value ) {
			$e_invoice->update_meta_data( $meta_key, $value );
		}

		do_action( "{$this->get_general_hook_prefix()}before_create_einvoice", $e_invoice, $this, $meta_data );

		$is_finalized = $this->is_finalized() ? true : false;
		$result       = $e_invoice->generate( $is_finalized ? false : true );

		if ( is_wp_error( $result ) ) {
			return $result;
		}

		$this->add_attachment( $e_invoice );

		if ( $is_finalized && $e_invoice->has_pdf_merge() && $this->has_file() ) {
			$merged_pdf_stream = $e_invoice->get_merged_pdf( $this->get_stream(), $result );
		...
		do_action( "{$this->get_general_hook_prefix()}after_create_einvoice", $e_invoice, $this, $meta_data );
	...
	public function get_pdf_attachment() {
		$pdf = $this->get_attachments( 'pdf' );

		if ( $pdf ) {
			return array_values( $pdf )[0];
		}

		return false;
	}

	public function has_pdf_attachment() {
		return $this->get_pdf_attachment() ? true : false;
	}
	 ...
	 */
	protected function update_pimary_file( $stream, $filename = '' ) {
		$error    = new WP_Error();
		$filename = empty( $filename ) ? $this->get_filename() : $filename;

		/**
		 * In case there is no original PDF attachment yet copy
		 * the current PDF file to a new file and store as separate attachment.
		 */
		if ( ! $this->get_pdf_attachment() && $this->has_file() ) {
			$attachment = new Attachment();
			$attachment->set_document( $this );
			$attachment->set_extension( 'pdf' );
			$attachment->set_type( 'pdf' );

			$path = sab_upload_document( $attachment->get_filename(), $this->get_stream(), true, true );

			if ( is_wp_error( $path ) ) {
				$error->add( 'render-einvoice-error', $path->get_error_message() );
				return $error;
			} else {
		...
		/**
		 * Upload (override) the current file with the newly created stream.
		 */
		$path = sab_upload_document( $filename, $stream, true, true );

		if ( is_wp_error( $path ) ) {
			$error->add( 'render-einvoice-error', $path->get_error_message() );

			return $error;
		} elseif ( $path !== $this->get_relative_path() ) {
	...
	public function get_payment_reference() {
		$reference    = apply_filters( "{$this->get_hook_prefix()}invoice_payment_reference", _x( '{document_title} Order {order_number}', 'storeabill-invoice-payment-reference', 'woocommerce-germanized-pro' ), $this );
		$placeholders = apply_filters(
			"{$this->get_hook_prefix()}invoice_payment_reference_placeholders",
			array(
				'{document_date}'             => wc_format_datetime( $this->get_date_created() ),
				'{document_formatted_number}' => $this->get_formatted_number(),
				'{document_number}'           => $this->get_number(),
				'{document_title}'            => $this->get_title(),
				'{document_type}'             => sab_get_document_type_label( $this->get_type() ),
				'{order_number}'              => $this->get_order_number(),
	...
	*/
	...
	public function set_base_currency( $currency ) {
		$this->set_prop( 'base_currency', $currency );
	}
	 ...
	 */
	public function set_exchange_rate( $rate ) {
		$this->set_prop( 'exchange_rate', $rate );
	}

	/**
	 * Set price formatting args.
	 *
	 * @param array $format The price formatting args
	 */
	public function set_price_formatting_args( $format ) {
		$this->set_prop( 'price_formatting_args', empty( $format ) ? array() : (array) $format );
	}
	...
	public function get_formatted_price( $price, $type = '' ) {
		...
		return sab_format_price( $price, $this->get_price_formatting_args() );
	}
