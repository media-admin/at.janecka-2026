...
namespace Vendidero\StoreaBill\Vendor\Einvoicing\Readers;
...
use Vendidero\StoreaBill\Vendor\Einvoicing\Traits\VatTrait;
...
class UblReader extends AbstractReader
{
    ...
    public function import(string $document): Invoice
    {
        ...
        foreach ($xml->getAll("{{$cac}}TaxTotal/{{$cac}}TaxSubtotal/{{$cac}}TaxCategory") as $node) {
            ...
            if ($exemptionReasonCodeNode === null && $exemptionReasonNode === null) {
                continue;
            }
            ...
            // Save reasons
            $taxExemptions[$key] = ["code" => $exemptionReasonCodeNode === null ? null : $exemptionReasonCodeNode->asText(), "reason" => $exemptionReasonNode === null ? null : $exemptionReasonNode->asText()];
        }
        ...
        if ($termsNode !== null) {
            ...
            if ($firstPayment !== null) {
                $firstPayment->setTerms($termsNode->asText(), true);
                // @phan-suppress-current-line PhanDeprecatedFunction
            }
        }
    ...
    private function parsePostalAddressFields(UXML $xml, $target)
    {
        ...
        $cbc = UblWriter::NS_CBC;
        // Postal address
        $addressNodes = array_filter([$xml->get("{{$cbc}}StreetName"), $xml->get("{{$cbc}}AdditionalStreetName"), $xml->get("{{$cac}}AddressLine/{{$cbc}}Line")]);
    ...
    private function addAllowanceOrCharge($target, UXML $xml, array &$taxExemptions)
    {
        ...
        if ($factorNode !== null) {
            ...
            throw new InvalidArgumentException('Missing both <cbc:Amount /> and <cbc:MultiplierFactorNumeric />' . ' nodes from allowance/charge');
        }
        // VAT attributes
