...
use DateTime;
use Vendidero\StoreaBill\Vendor\Einvoicing\AllowanceOrCharge;
...
use Vendidero\StoreaBill\Vendor\Einvoicing\Models\InvoiceTotals;
use Vendidero\StoreaBill\Vendor\Einvoicing\Party;
use Vendidero\StoreaBill\Vendor\Einvoicing\Payments\Card;
use Vendidero\StoreaBill\Vendor\Einvoicing\Payments\Mandate;
use Vendidero\StoreaBill\Vendor\Einvoicing\Payments\Payment;
use Vendidero\StoreaBill\Vendor\Einvoicing\Payments\Transfer;
use Vendidero\StoreaBill\Vendor\UXML\UXML;
...
class UblWriter extends AbstractWriter
{
    ...
    public function export(Invoice $invoice): string
    {
        ...
        $xml = UXML::newInstance($rootElementName, null, ['xmlns' => $isCreditNoteProfile ? self::NS_CREDIT_NOTE : self::NS_INVOICE, 'xmlns:cac' => self::NS_CAC, 'xmlns:cbc' => self::NS_CBC]);
        ...
        // BT-20: Payment terms
        $firstPayment = $invoice->getPayment(true);
        // @phan-suppress-current-line PhanDeprecatedFunction
        $paymentTerms = $invoice->getPaymentTerms() ?? ($firstPayment === null ? null : $firstPayment->getTerms(true));
        // @phan-suppress-current-line PhanDeprecatedFunction
    ...
    private function isCreditNoteProfile(Invoice $invoice): bool
    {
        ...
        return in_array($type, [Invoice::TYPE_CREDIT_NOTE_RELATED_TO_GOODS_OR_SERVICES, Invoice::TYPE_CREDIT_NOTE_RELATED_TO_FINANCIAL_ADJUSTMENTS, Invoice::TYPE_CREDIT_NOTE, Invoice::TYPE_FACTORED_CREDIT_NOTE, Invoice::TYPE_FORWARDERS_CREDIT_NOTE]);
    }
    /**
     * Add identifier node
    ...
    private function addOrderReferenceNode(UXML $parent, Invoice $invoice)
    {
        ...
        if ($purchaseOrderReference === null && $salesOrderReference === null) {
            return;
        }
     ...
     */
    private function addVatNode(UXML $parent, string $name, string $category, ?float $rate, ?string $exemptionReasonCode = null, ?string $exemptionReason = null)
    {
        $xml = $parent->add($name);
        // VAT category
     ...
     */
    private function addAllowanceOrCharge(UXML $parent, AllowanceOrCharge $item, bool $isCharge, Invoice $invoice, ?InvoiceTotals $totals, ?InvoiceLine $line)
    {
        ...
        // Amount
        $baseAmount = $atDocumentLevel ? $totals->netAmount : $line->getNetAmountBeforeAllowancesCharges() ?? 0.0;
        // @phan-suppress-current-line PhanPossiblyNonClassMethodCall
        $this->addAmountNode($xml, 'cbc:Amount', $invoice->round($item->getEffectiveAmount($baseAmount), 'line/allowanceChargeAmount'), $invoice->getCurrency());
        // Base amount
        if ($item->isPercentage()) {
            $this->addAmountNode($xml, 'cbc:BaseAmount', $invoice->round($baseAmount, 'line/netAmount'), $invoice->getCurrency());
        }
        // Tax category
        if ($atDocumentLevel) {
    ...
    private function addTaxTotalNodes(UXML $parent, InvoiceTotals $totals)
    {
        ...
        foreach ($totals->vatBreakdown as $item) {
            ...
            $this->addVatNode($vatBreakdownNode, 'cac:TaxCategory', $item->category, $item->rate, $item->exemptionReasonCode, $item->exemptionReason);
        }
        // Add tax amount in VAT accounting currency (if any)
        $customVatAmount = $totals->customVatAmount;
        if ($customVatAmount !== null) {
            $this->addAmountNode($parent->add('cac:TaxTotal'), 'cbc:TaxAmount', $customVatAmount, $totals->vatCurrency ?? $totals->currency);
        }
    }
    /**
     ...
     */
    private function addLineNode(UXML $parent, InvoiceLine $line, Invoice $invoice, bool $isCreditNoteProfile, int &$lastGenId, array &$usedIds)
    {
        ...
        if ($netAmount !== null) {
            $this->addAmountNode($xml, 'cbc:LineExtensionAmount', $invoice->round($netAmount, 'line/netAmount'), $invoice->getCurrency());
        }
        ...
        $price = $line->getPrice();
        if ($price !== null) {
            $this->addAmountNode($priceNode, 'cbc:PriceAmount', $invoice->round($price, 'line/price'), $invoice->getCurrency());
        }
        // Base quantity
