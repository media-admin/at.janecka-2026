...
class Models extends REST {
	...
	public function get_guidance_by_tax_rule( $tax_rule ) {
		$result = $this->get_sync_helper()->parse_response( $this->get( 'ReceiptGuidance/forTaxRule', array( 'taxRule' => $tax_rule ) ) );

		return $result;
	}

	public function get_bookkeeping_version() {
		$result  = $this->get_sync_helper()->parse_response( $this->get( 'Tools/bookkeepingSystemVersion' ) );
		$version = '1.0';

		if ( ! is_wp_error( $result ) ) {
			$version = $result->get_body()['objects']['version'];
		}
	...
	public function get_categories() {
		...
		if ( $this->sync_helper->has_enhanced_bookkeeping_mode() ) {
			$result = $this->get_sync_helper()->parse_response( $this->get( 'Account/Factory/selectableAccounts?semanticVatRules=true' ) );

			if ( ! is_wp_error( $result ) ) {
				foreach ( $result->get_body()['objects'] as $category ) {
					if ( ! in_array( 'RECEIPT_REVENUE', $category['allowedDocumentTypes'], true ) ) {
						continue;
					}

					$category['id']   = $category['accountDatevId'];
					$category['name'] = $category['accountName'];
					$category['type'] = $category['accountGuideType'];

					$category['accountingSystemNumber'] = array(
						'number' => $category['accountNumber'],
					);
			...
			if ( ! is_wp_error( $result ) ) {
				$objects    = array_filter( (array) $result->get_body()['objects'] );
				$categories = array_merge( $categories, $objects );
			}
	...
	public function upload_voucher_file( $file, $voucher_type = 'D' ) {
		try {
			$mime_type = $this->get_file_mime_type( $file );
			$curl_file = new \CURLFile( $file, $mime_type );
