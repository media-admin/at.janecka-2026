...
class Helper {
	...
	protected static function parse_tax_class_slug_names( $tax_class_slug_names = array() ) {
		return wp_parse_args(
			...
			array(
				...
				'greater-reduced' => apply_filters( 'woocommerce_eu_tax_helper_tax_class_greater_reduced_name', _x( 'Greater reduced rate', 'tax-helper-tax-class-name', 'woocommerce-germanized-pro' ) ),
				'super-reduced'   => apply_filters( 'woocommerce_eu_tax_helper_tax_class_super_reduced_name', _x( 'Super reduced rate', 'tax-helper-tax-class-name', 'woocommerce-germanized-pro' ) ),
		...
		$rates = array(
			...
			'PT' => array(
				array(
					...
					'standard' => 22,
					'reduced'  => array( 4, 12 ),
					'name'     => _x( 'Madeira', 'tax-helper', 'woocommerce-germanized-pro' ),
				),
				array(
					// Acores
					...
					'standard' => 18,
					'reduced'  => array( 4, 9 ),
					'name'     => _x( 'Acores', 'tax-helper', 'woocommerce-germanized-pro' ),
				),
				array(
					'standard' => 23,
					'reduced'  => array( 6, 13 ),
			...
			'GB' => array(
				array(
					...
					'reduced'  => array( 5 ),
					'postcode' => array( 'BT*' ),
					'name'     => _x( 'Northern Ireland', 'tax-helper', 'woocommerce-germanized-pro' ),
				),
			),
		);
		...
		foreach ( self::get_vat_postcode_exemptions_by_country() as $country => $exempt_postcodes ) {
			if ( array_key_exists( $country, $rates ) ) {
				...
				$postcode_exempt = array(
					...
					'name'      => _x( 'Exempt', 'tax-helper-rate-import', 'woocommerce-germanized-pro' ),
					'is_exempt' => true,
				);
	...
	public static function import_rates( $rates, $tax_class = '', $tax_class_type = '', $clean = true ) {
		...
		foreach ( $rates as $rate ) {
			...
			$tax_rate_name = apply_filters( 'woocommerce_eu_tax_helper_import_tax_rate_name', sprintf( _x( 'VAT %1$s %% %2$s', 'tax-helper-rate-import', 'woocommerce-germanized-pro' ), $vat_rate, $vat_desc ), $rate['rate'], $iso, $tax_class, $rate );
